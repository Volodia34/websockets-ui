...
function populateBoardWithShips(board, ships, shipStates) {
...
export function initializeGameData(room) {
...
function markCellsAroundSunkShip(board, ship) {
...
export function applyAttack(roomId, attackingPlayerId, x, y) {
    ...
    if (!opponentBoard || !opponentShipStates)
        return null;
    ...
    let sunkShipCellsForResponse = undefined;
    if (cellState === 0) {
        opponentBoard[y][x] = 2;
        attackStatus = "miss";
    }
