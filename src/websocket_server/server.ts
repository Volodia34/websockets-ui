...
wss.on('connection', (ws: WebSocket, req: http.IncomingMessage) => {
    ...
    ws.on('message', (message: Buffer) => {
        ...
        try {
            clientMsg = JSON.parse(messageString) as ClientMessage;
        } catch (err) {
    ...
    ws.on('close', (code, reason) => {
        ...
        if (closedPlayerId) {
            const roomContainingPlayer = gameRoomsDB.find(room =>
                room.roomUsers.some(user => user.index === closedPlayerId) && room.isGameActive
            );
