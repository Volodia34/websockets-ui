...
export class GameService {
    ...
    public addShips(
        ...
        if (updatedRoom.shipsReadyCount === 2) {
            ...
            const boardSize = 10;
            const initialBoard = (): GameBoard => Array(boardSize).fill(null).map(() => Array(boardSize).fill(0 as CellStatus));
            ...
            if (sunkShipData) {
                ...
                for (let i = 0; i < sunkShipData.length; i++) {
                    ...
                    if (partX >= 0 && partY >= 0 && partY < targetBoard.length && partX < targetBoard[0].length) {
                        targetBoard[partY][partX] = 4 as CellStatus;
                    }
                }
            }
        }
                ...
                for (let dy = -1; dy <= 1; dy++) {
                    ...
                    if (checkX >= 0 && checkX < boardWidth && checkY >= 0 && checkY < boardHeight) {
                        if (board[checkY][checkX] === 0) {
                            board[checkY][checkX] = 2 as CellStatus;
                        }
                    }
                }
            }
    ...
    public cleanupGame(gameId: string, connectionId?: string): void {
