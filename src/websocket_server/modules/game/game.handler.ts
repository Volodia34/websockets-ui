...
export class GameHandler {
    ...
    public handleAddShips(
        ...
        if (result.room && result.gameCanStart && result.startGameData) {
            ...
            if (room.currentPlayerTurn) {
                const turnUpdatePayload: TurnDataToClient = {
    ...
    public handleAttack(
        ...
        const room = result.room;
        if (room) {
            const attackResponsePayload: AttackResponseData = {
    ...
    public handleRandomAttack(
        ...
        const gameBoards = (gameServiceInstance as any).activeGameBoards.get(clientData.gameId) as { board1: GameBoard; board2: GameBoard } | undefined;
        if (!gameBoards) {
        ...
        const attackData: AttackClientData = {
            gameId: clientData.gameId,
